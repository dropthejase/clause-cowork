...
import { listTags, addTag, updateTag, deleteTag, importTags, exportTags, Section } from "@word-graph/shared";
import type { PoolTag, TagKind } from "@word-graph/shared";
import { FONT } from "@word-graph/shared";
import { api } from "../../api";

async function restoreDefaults(workspacePath: string, kind: TagKind): Promise<number> {
  const res = await api.post(`/tags/${encodeURIComponent(workspacePath)}/restore-defaults?kind=${kind}`);
  return res.data.restored as number;
}

const KIND_META: Record<TagKind, { label: string; description: string; addPlaceholder: string; canRestore: boolean }> = {
  doc_type: {
    label: "Document Types",
    description: "Categorise whole documents (e.g. Employment Agreement, NDA). The agent assigns one type per document.",
    addPlaceholder: "Type name (e.g. Joint Venture)",
    canRestore: true,
  },
  doc_tag: {
    label: "Document Tags",
    description: "Topic labels applied to whole documents (e.g. Executed, Confidential). The agent can assign multiple tags per document.",
    addPlaceholder: "Tag name (e.g. Executed)",
    canRestore: false,
  },
  clause_type: {
    label: "Clause Types",
    description: "Categorise individual clauses (e.g. Obligation, Definition). The agent assigns one type per clause.",
    addPlaceholder: "Type name (e.g. Warranty)",
    canRestore: true,
  },
  clause_tag: {
    label: "Clause Tags",
    description: "Topic labels applied to individual clauses (e.g. liability-cap, auto-renewal). The agent can assign multiple tags per clause.",
    addPlaceholder: "Tag name (e.g. auto-renewal)",
    canRestore: false,
  },
};
...
export function DocTagPoolPage({ kind = "doc_type", strict = true, onStrictChange }: Props) {
  ...
  async function handleRestoreDefaults() {
    ...
    setRestoreMsg(null);
    try {
      const n = await restoreDefaults(docPath, kind);
      setRestoreMsg(n === 0 ? "All defaults already present." : `${n} default${n > 1 ? "s" : ""} restored.`);
      await loadTags();
    } catch {
  ...
  return (
    <Section label={meta.label} badge={<span style={{ fontSize: FONT.md, color: theme.muted }}>{tags.length}/100</span>}>
      {/* Description */}
      <div style={{ padding: "8px 13px", borderBottom: `1px solid ${theme.edgeBorder}`, background: theme.white }}>
        <p style={{ margin: 0, fontSize: FONT.sm, color: theme.muted, lineHeight: 1.5 }}>{meta.description}</p>
      </div>

      {/* Strict mode toggle */}
      {onStrictChange && (
        <div style={{ padding: "8px 13px", borderBottom: `1px solid ${theme.edgeBorder}`, display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: FONT.md, color: theme.muted, flex: 1 }}>Lock — agent may only use entries from this list</span>
          <button
      ...
      <div style={{ padding: "6px 13px", borderBottom: `1px solid ${theme.base}`, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        {meta.canRestore && (
          <button onClick={handleRestoreDefaults} style={{ ...btnSecondary, fontSize: FONT.sm }}>
            Restore defaults
          </button>
        )}
        <div style={{ flex: 1 }} />
        <input ref={fileInputRef} type="file" accept=".csv" style={{ display: "none" }} onChange={handleFileImport} />
        <button onClick={() => setShowImportInfo((v) => !v)} style={btnSecondary}>Import CSV</button>
      ...
      {/* Import CSV info box */}
      {showImportInfo && (
        <div style={{ padding: "10px 13px", borderBottom: `1px solid ${theme.edgeBorder}`, background: theme.graphBg }}>
          <p style={{ margin: "0 0 6px", fontSize: FONT.sm, color: theme.black, fontWeight: 600 }}>CSV format</p>
          <p style={{ margin: "0 0 8px", fontSize: FONT.sm, color: theme.muted }}>
            Two columns: <code>tag</code> and <code>description</code>. Tag names max 64 characters; descriptions max 256. Max 100 entries per pool.
          </p>
          <pre style={{ margin: "0 0 10px", fontSize: FONT.sm, background: theme.white, padding: "8px 10px", borderRadius: 4, border: `1px solid ${theme.edgeBorder}`, color: theme.black, boxShadow: "inset 0 1px 3px rgba(0,0,0,0.06)" }}>
{`tag,description
Employment,Employment contracts and offer letters
M&A,Mergers acquisitions and share purchase agreements`}
          </pre>
          <button onClick={() => fileInputRef.current?.click()} style={btnPrimary}>Choose file</button>
        </div>
      )}

      {restoreMsg && (
        <div style={{ padding: "4px 13px", fontSize: FONT.sm, color: theme.charcoal, background: theme.base, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span>{restoreMsg}</span>
          <button onClick={() => setRestoreMsg(null)} style={{ background: "none", border: "none", cursor: "pointer", color: theme.muted }}>×</button>
        </div>
      )}
        ...
        <div style={{ padding: "6px 13px", borderBottom: `1px solid ${theme.base}`, display: "flex", alignItems: "center", gap: 8, background: theme.graphBg }}>
          ...
          {confirmBulkDelete && (
            <>
              ...
              <button onClick={() => setConfirmBulkDelete(false)} style={{ ...btnSecondary }}>No</button>
            </>
          )}
        </div>
      )}
      ...
      <div style={{ padding: "6px 13px", display: "flex", flexDirection: "column", gap: 6 }}>
        {tags.length === 0 && !loading && (
          <div style={{ fontSize: FONT.md, color: theme.muted, padding: "4px 0" }}>
            No entries yet. Add one below or import a CSV.
          </div>
        )}
        {sortedTags.map((t) => (
      ...
      <div style={{ padding: "8px 13px", borderTop: `1px solid ${theme.edgeBorder}`, display: "flex", flexDirection: "column", gap: 6 }}>
        <div style={{ fontSize: FONT.sm, color: theme.muted }}>
          Name: max 64 characters. Description: max 256 characters.
        </div>
        ...
        <div>
          <textarea
            ...
            placeholder="Description — helps agents and users understand this category"
            value={newDesc}
            maxLength={256}
            rows={2}
        ...
        {tags.length >= 100 && (
          <div style={{ fontSize: FONT.sm, color: "#b91c1c" }}>Pool is at the 100-entry limit. Remove an entry before adding more.</div>
        )}
      </div>
    </Section>
  );
